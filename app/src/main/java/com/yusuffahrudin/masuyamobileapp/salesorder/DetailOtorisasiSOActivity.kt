...
class DetailOtorisasiSOActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.containerSo.offscreenPageLimit = 2
    ...
    fun setView(){
        ...
        val fragmentItem = binding.containerSo.adapter?.instantiateItem(binding.containerSo, 1) as OtorisasiSOItemFragment
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (id == android.R.id.home) {
            ...
            if(listHeader[0].statusorder == "OPEN"){
                ...
                DialogAlert("Add item tidak diperbolehkan pada SO pending/close", "attention", this@DetailOtorisasiSOActivity)
            }
            ...
            if(listHeader[0].statusorder == "OPEN" || listHeader[0].statusorder == "PENDING"){
                if(aksesSaveOrEdit){
                    ...
                    DialogAlert(getString(R.string.tidak_mempunyai_hak_akses), "attention", this@DetailOtorisasiSOActivity)
                }
            } else {
                DialogAlert("SO sudah closed", "attention", this@DetailOtorisasiSOActivity)
            }
        }
    ...
    private fun setupViewPager(viewPager: ViewPager) {
        ...
        adapter.addFragment(OtorisasiSOHeaderFragment(), "Header")
        adapter.addFragment(OtorisasiSOItemFragment(), "Item")
    ...
    private fun cekParameter(): Boolean{
        ...
        val hasil: Boolean = if (listHeader.size == 0){
            DialogAlert("Header tidak ada data, isi dulu..", "error", this@DetailOtorisasiSOActivity)
            false
        } else {
            when {
                listHeader[0].kdcust == "" -> {
                    DialogAlert("Field customer masih kosong, diisi dulu..", "error", this@DetailOtorisasiSOActivity)
                    false
                }
                listHeader[0].kdgd.equals("gudang", true) -> {
                    DialogAlert("Field gudang belum dipilih, lebih teliti lah..", "error", this@DetailOtorisasiSOActivity)
                    false
                }
                else -> true
            }
    ...
    private fun dialogPilihBarang(){
        ...
        if(cekParameter()){
            ...
            bindingDialogPilihBarang.tvAddPrice.setOnClickListener {
                cekAkses()
                if (isadd) {
                    val intent = Intent(this@DetailOtorisasiSOActivity, CreatePriceCust::class.java)
                    ...
                    DialogAlert("anda tidak mempunyai hak akses", "error", this@DetailOtorisasiSOActivity)
                }
            }
    ...
    private inner class AdapterBarang(private val activity: Activity, private var list_tampung: ArrayList<ItemJual>) : RecyclerView.Adapter<AdapterBarang.ViewHolder>(){
        ...
        private fun dialogQty() {
            ...
            val bottomLayout = LayoutInflater.from(this@DetailOtorisasiSOActivity).inflate(R.layout.dialog_so_qty_item, null)
            ...
            API.instance().create(ApiService::class.java)
                    ...
                    ?.enqueue(object : Callback<Result?> {
                        override fun onFailure(call: Call<Result?>, e: Throwable) {
                            ...
                            DialogAlert(e.message, "error", this@DetailOtorisasiSOActivity)
                        ...
                        override fun onResponse(call: Call<Result?>, response: retrofit2.Response<Result?>) {
                            if (response.isSuccessful) {
                                ...
                                if (response.body()?.success == 1) {
                                    DialogAlert(response.body()?.message, "success", this@DetailOtorisasiSOActivity)
                                    val fragmentHeader = binding.containerSo.adapter?.instantiateItem(binding.containerSo, 0) as SOHeaderFragment
                                    ...
                                    DialogAlert(response.body()?.message, "error", this@DetailOtorisasiSOActivity)
                                }
                            }
                        }
                    })
                        ...
                        override fun onResponse(call: Call<Result?>, response: retrofit2.Response<Result?>) {
                            if (response.isSuccessful) {
                                ...
                                if (response.body()?.success == 1) {
                                    ...
                                    if (otoCust && otoHarga) {
                                        println("================ cust & harga")
                                        notif.pushNotif(this@DetailOtorisasiSOActivity, kdkota,
                                    ...
                                    binding.tvProgressbar.visibility = View.GONE
                                    window.clearFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
                                    DialogAlert(response.body()?.message, "success-reply", this@DetailOtorisasiSOActivity)
                                    listHeader.clear()
                                    listCart.clear()
                                    listChartLama.clear()
                                } else {
                        ...
                        override fun onResponse(call: Call<String?>, response: retrofit2.Response<String?>) {
                            if (response.code() == 200) {
                                ...
                                if (message.equals("Customer has been blacklisted!", true)) {
                                    binding.progressbar.visibility = View.GONE
                                    window.clearFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
                                    DialogAlert(message, "error", this@DetailOtorisasiSOActivity)
                                } else {
